...
exports.listener=function(request, response){
	...
	function query() {
		...
		async.series({
											...
											if(results2.length > 0){
												...
												for(var i = 0; i < results2.length; i++){
													...
													if(results2[i].type == "android"){
														...
														androidPush.push("gallery", results2[i].key_num, connectorName, brd_title, connectorNo, brd_no[0].brd_no, "", results2[i].popup_yn);
													} else if(results2[i].type == "ios"){
														console.log("results2[i].type == " + results2[i].type + " >> IOS");
														ios_push_url = "/board_view?brd_no=" + brd_no[0].brd_no;
														iosPush.push("gallery", results2[i].key_num, connectorName, brd_title, connectorNo, brd_no[0].brd_no, "", results2[i].popup_yn);
													} else {
			...
			response.end('<script>location.href = "gallery_board_list?category='+category+'"</script>');
		 });			
	}	
		...
		if(filesArray.length > 0) {
			...
			for(var i = 0; i < filesArray.length; i++) {
				...
				db.query(file_sql, function(error, results){
					if(error){
						console.log("/gallery_board_save/file/error===", file_sql);
						response.send("fail");
					} 
				});
			} 
