...
exports.listener=function(request, response){
	...
	var brdlink4 =  request.param("brdlink4") == undefined ? "" : request.param("brdlink4");
	var brdlink5 =  request.param("brdlink5") == undefined ? "" : request.param("brdlink5");
	var brdlink6 =  request.param("brdlink6") == undefined ? "" : request.param("brdlink6");
	var brdlink7 =  request.param("brdlink7") == undefined ? "" : request.param("brdlink7");

	var category = request.param('category') == null ? 'notice' : request.param('category');
	var board_table;

	console.log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
	console.log(brd_create_id);
	console.log(connectorNo);
	switch (category) {
		case 'notice':
			board_table = 'board';
			brd_id = 'notice';
		...
		case 'event':
			...
			break;
		case 'sub_notice':
			board_table = 'sub_notice_board';
			brd_id = 'sub_notice';
			break;
		case 'sub_media':
			board_table = 'sub_media_board';
			brd_id = 'sub_media';
			break;
	...
	function query() {
		...
		async.series({
			...
			response.end('<script>location.href = "board_list?category='+category+'"</script>');
		 });			
	}	
