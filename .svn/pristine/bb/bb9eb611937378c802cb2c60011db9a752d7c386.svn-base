...
exports.listener=function(request, response){
	...
	var table = "free_board";
	var file_table = "free_files";
	var date = new Date();
	...
	var sort = request.param("sort") == undefined ? "" : request.param("sort");
	//var brd_create_id = request.session.ad_id;
	var brd_create_id = (connectorNo=="5555555") ? "'"+request.session.ad_id+"'" : "(select orgmem_name from orgmember where orgmem_no="+connectorNo+")";
	var board_no = "";
	var brd_create_date = "";
		...
		async.series({
			 first : function(callback){
				db.query(sql, function(error, results){
					if(error){
						response.send("fail");
						console.log("user/free_board_save/error=====", sql);
					}else{
						if(mode == "write"){
			...
			response.end('<script>location.href = "free_board_list"</script>');
		 });			
	}	
