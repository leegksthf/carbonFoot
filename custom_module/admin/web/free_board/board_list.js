...
exports.listener=function(request, response){
	...
	if(request.session.admin_yn=='Y'){
		fs.readFile("./views/admin/web/free_board/board_list.html", "utf-8", function(error, data){
			page_navi.action('free', p_where, nowPage, col_count, row_count, function(error, page_txt, page_start, page_end, total_cnt){
			...
			db.query(sql, function(error, results){
				if(error){
					response.send("fail");
					console.log("admin/web/free_board_list/error======", sql);
				}else{
