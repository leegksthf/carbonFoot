...
exports.listener=function(request, response){
	...
	var orgmem_no = request.param("orgmem_no") == "" ? -1 : request.param("orgmem_no");
		...
		async.series({
			 first : function(callback){
				db.query(sql, function(error, results){
					if(error){
						response.send("fail");
						console.log("admin/web/gallery_board_save/error=====", sql);
					}else{
						if(mode == "write"){
							...
							db.query(select_sql, function(error, brd_no){
								if(error){
									...
									if(brdPushYn_result == "Y"){
										var push_sql = "select key_num, type, popup_yn from push where phone_num != ? and popup_yn = 'y' ";
											...
											if(results2.length > 0){
												...
												for(var i = 0; i < results2.length; i++){
													...
													if(results2[i].type == "android"){
														...
														console.log("results2[i].type == " + results2[i].type + " >> IOS "  + results2[i].orgmem_phone);
														ios_push_url = "/gallery_board_view?brd_no=" + brd_no[0].brd_no;
		...
		if(filesArray.length > 0) {
			...
			for(var i = 0; i < filesArray.length; i++) {
				...
				db.query(file_sql, function(error, results){
					if(error){
						console.log("admin/web/gallery_board_save/file/error===", file_sql);
						response.send("fail");
					} 
				});
			} 
